...
class Parser{
    public:
        ...
        static Variable* readGroundedVar( StateDescriptor *sd, const string &name, Instance *ins ){
            ...
            bool var_constant = isDigit( name );    //是否名字为数字来判断
            ...
            if( var_constant ) {
            ...
            else if( (pos = name.find('(')) != string::npos ) {       //If the name contains a '(', it indicates that the variable is a predicate. 
                                                                        //It sets the variable type to "predicate" and extracts the variable name before the '('
                var_type = "predicate";
                ...
                for (unsigned i = 0; i < param_names.size(); i++) {
                    auto obj_address = ins->getObjectAddress(param_names[i]);                //这是两个函数的不同点
        ...
        //It appends the '(' to the variable name and splits the substring between the '(' and ')' into parameter names using the split function. It iterates over the parameter names, finds the corresponding index in the par_name_to_idx map, and gets the grounded parameter from the grounded_params vector at that index. It then gets the pointer ID using the getTypeID function of the StateDescriptor object and adds it to the param_ids vector. It also appends the pointer name to the variable name.
        ...
        static bool parseInstance( Domain *dom, Instance *ins, const string &file_name = "", int instance_id = 0 ){
            ...
            while( !ifs.eof() && word == "(" ){
                ...
                if( pos != string::npos ) { // it is a predicate
                    ...
                    //init->setPointer(sd, pointer_name, value, type? );

                }

                ifs >> word;
                if( word != ")" ){
                    cerr << errorMessage( "(INIT", word );
                    return false;
                }
                ifs >> word;
            }
            ...
            while( !ifs.eof() && word == "(" ){
                ...
                ifs >> word;
                if( word != ")" ){
                    cerr << errorMessage( "(GOAL", word );
                    return false;
                }
                ifs >> word;
            }

            // 4. Parse epistemic goal condition
            if (word != "EGOAL:" && word != ")") {
                cerr << errorMessage( "EGOAL:", word ) << endl;
                return false;
            }
            ...
            ifs >> word;
             ...
             // Assuming the loop condition is correct, no change needed here.
           while (!ifs.eof() && word == "(") {
                std::string goal_statement;
                std::string value;
                
                // Read until '=' to get the goal_statement
                std::getline(ifs, goal_statement, '@');
                
                // Read the value directly
                ifs >> value;
                cout<<"goal_statement"<<goal_statement<<"value"<<value<<endl;
                // Store in map
                ins-> addEGoal(goal_statement, convertIntToPDDL_TERNARY(stoi(value)));
				
				ifs >> word;
                if( word != ")" ){
                    cerr << errorMessage( "(EGOAL", word );
                    return false;
                }
                ifs >> word;
            }
